...
if __name__ == "__main__":
	...
	# rnaseq data, it is qnormed i believe
	...
	df = df.loc[:, df_ref.stack().values] # leave only test and control samples
	
	# ppi
	ppi = pd.read_csv('ppi_19k.csv')
	...
	df_wk0toend_individual.to_csv('wk0toend_ind_zeromeanln_8kppi.csv')
	...
	ax.set_title('global per-sample entropy on tf-ppi (7.8k pairs) using zeromean-ln rnaseq data. s4 died on wk7, s3 on 11 and s6 on wk16')
	ax.legend()
	plt.savefig('wk0toend_ind_zeromeanln_8kppi.png')
